...
export default function Updateform() {
    ...
    const handleConfirmEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      useEffect(() => {
        const id = router.query.id as string;
        if (id) {
          fetch(`/api/hello/${id}`) // Replace this with the correct API endpoint to fetch the account data
            .then((response) => response.json())
            .then((data) => {
              setNom(data.nom);
              setPrénom(data.prenom);
              setCivilité(data.civilite);
              setSexe(data.sexe);
              setSpécialité(data.specialite);
              setMode(data.mode_exercice);
              setannee(data.annee_exercice);
              setPays(data.pays);
              setCodepostal(data.code_postale);
              setRPPS(data.rpps);
      ...
      function updateAcount(id: string) {
        ...
        const acount = {
            id,nom, prenom, civilite, sexe, specialite, mode_exercice, annee_exercice, pays,
            code_postale, rpps, pseudo, email, password,
        }
        console.log("ID:", id);
        fetch(`/api/updateAcount/{id}`, {
            method: "PUT",
            headers: {
            ...
            .then((response) => {
                if (response.ok) {
                    ...
                    console.error("Failed to update account", acount);
                }
            })
            .catch((error) => {
                console.error("Error updating account:", acount);
            });
    }
    return (
        <>
                    ...
                    <form>
                        <div className="info-utilisateur">
                            <div className="input-acount">
                                <label  htmlFor="input-field">Nom</label>
